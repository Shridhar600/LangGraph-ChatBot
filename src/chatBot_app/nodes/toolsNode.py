...
class BasicToolNode:
    ...
    def __call__(
        self, inputs: dict
    ):  # This makes the class callable, so we can use it like a function.
        ...
        return {
            "messages": outputs
        }  # Output is a list of ToolMessage objects. This Will contain content, of all the tools called in the last message.
...
def route_tools(state: any) -> str:
    ...
    ):  # Check if the last message has tool calls and also check if the tool calls are not empty.
        return "tools"
    return END
